...
def main():
    ...
    picp_var1 = []
    picp_var2 = []
    phi = pd.read_csv("~/Desktop/indicator_deep_kriging/Bivariate_DeepKriging-main/src/python_scripts/phi.csv", sep = ",")
    phi_train,phi_test = train_test_split(pd.DataFrame.to_numpy(phi), test_size = 0.1, random_state=123)
    for sim in range(num_sim):
        df_train = pd.read_csv("~/Desktop/indicator_deep_kriging/Bivariate_DeepKriging-main/synthetic_data_simulations_non-Stationary/training_data/2D_nonstationary_1200_"+str(sim+1)+"-train.csv", 
                             sep = ",")
        df_test = pd.read_csv("~/Desktop/indicator_deep_kriging/Bivariate_DeepKriging-main/synthetic_data_simulations_non-Stationary/testing_data/2D_nonstationary_1200_"+str(sim+1)+"-test.csv", 
                             sep = ",")
...
#         base_model.summary()
    ...
    df_mpiw.to_csv("~/Desktop/indicator_deep_kriging/Bivariate_DeepKriging-main/plot_results/DeepKriging_nonstationary_interval.csv", index = False)
