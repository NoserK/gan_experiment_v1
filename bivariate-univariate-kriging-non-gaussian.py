...
def generate_samples(n_samples, is_bivariate=True, seed=4):
    ...
    if is_bivariate:
        # Existing bivariate Gaussian process code
        cross_cov = np.array([[1.0, 0.5], [0.5, 1.0]])
        spatial_cov = generate_covariance_matrix(locations)
        
        # Generate correlated spatial random effects
        spatial_effects = np.zeros((len(locations), 2))
        for i in range(2):
            spatial_effects[:,i] = np.random.multivariate_normal(
                np.zeros(len(locations)), 
                spatial_cov
            )
        
        # Add cross-correlation
        spatial_effects = spatial_effects @ np.linalg.cholesky(cross_cov)
        ...
        # Existing univariate Gaussian process code
        spatial_cov = generate_covariance_matrix(locations)
        # Univariate case
...
def run_simulation(data_path, is_bivariate=True):
    ...
    # Save metrics
    metrics_filename = f"{data_path}-prediction_metrics.csv"
    ...
    summary_filename = f"{data_path}-summary_metrics.csv"
...
def main():
    ...
    for sim in range(n_simulations):
        ...
        # Save bivariate data
        train_biv.to_csv(f"synthetic_data/synthetic_bivariate/2d_nongaussian_{sim+1}-train.csv", index=False)
        test_biv.to_csv(f"synthetic_data/synthetic_bivariate/2d_nongaussian_{sim+1}-test.csv", index=False)
        
        print("Running bivariate simulation...")
        predictions_biv, ci_lower_biv, ci_upper_biv, y_test_biv, mse_biv, _, _ = run_simulation(
            f"synthetic_data/synthetic_bivariate/2d_nongaussian_{sim+1}",
            is_bivariate=True
        )
        ...
        train_uni.to_csv(f"synthetic_data/synthetic_univariate/2d_nongaussian_{sim+1}-train.csv", index=False)
        test_uni.to_csv(f"synthetic_data/synthetic_univariate/2d_nongaussian_{sim+1}-test.csv", index=False)
        
        print("Running univariate simulation...")
        predictions_uni, ci_lower_uni, ci_upper_uni, y_test_uni, mse_uni, _, _ = run_simulation(
            f"synthetic_data/synthetic_univariate/2d_nongaussian_{sim+1}",
            is_bivariate=False
        )
    ...
    # Save MSE results
    pd.DataFrame(mse_results_biv).to_csv("results/bivariate_mse.csv", index=False)
    pd.DataFrame({'mse': mse_results_uni}).to_csv("results/univariate_mse.csv", index=False)
