...
"""
Bivariate DeepKriging — Non-Gaussian with Covariates
...
# ─────────────────────── Wendland Basis ─────────────────────────
def build_wendland_basis(s, num_basis):
    """Build multi-resolution Wendland basis matrix."""
    N = len(s)
...
def train_model(phi, y, loss_fn, sim_iteration, loss_name):
    ...
    # Phase 1: warm-up
    model.fit(phi, y, epochs=1000, batch_size=512, verbose=0)
...
def print_full_summary(results_std, results_wt, num_sim):
    ...
    print()
    print("=" * 82)
    print(f"  FULL METRIC SUMMARY — NON-GAUSSIAN + COVARIATES  ({num_sim} simulations)")
    print("=" * 82)
...
def main():
    print()
    print("#" * 70)
    print("# NON-GAUSSIAN + COVARIATES: Standard MSE vs Weighted MSE Comparison")
    print("#" * 70)
    print()
    ...
    for sim in range(num_sim):
        ...
        df_train = pd.read_csv(
            f"{base_path}/synthetic_data_simulations_nonGaussian_cov/training_data/"
            f"2D_nonGaussian_1200_{sim+1}-train.csv", sep=",")
        df_test = pd.read_csv(
            f"{base_path}/synthetic_data_simulations_nonGaussian_cov/testing_data/"
            f"2D_nonGaussian_1200_{sim+1}-test.csv", sep=",")
        ...
        phi_train = build_wendland_basis(s_train, num_basis)
        ...
        phi_test = build_wendland_basis(s_test, num_basis)
        covariates_test = scaler.transform(
            np.array(df_test[["cov1", "cov2", "cov3", "cov4", "cov5"]]))
    ...
    df_std.to_csv(f"{save_path}/DeepKriging_nongaussian-cov_metrics_standard.csv", index=False)
    df_wt.to_csv(f"{save_path}/DeepKriging_nongaussian-cov_metrics_weighted.csv", index=False)
